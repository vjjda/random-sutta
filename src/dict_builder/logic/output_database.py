...
class OutputDatabase:
    ...
    def _create_tables(self):
        # ... (Bảng metadata, deconstructions, lookups giữ nguyên) ...
        ...
        # [UPDATED] Schema cho entries
        if self.config.is_tiny_mode:
            # Tiny: Chỉ 1 cột json definition
            self.cursor.execute("""
            ...
            # Mini/Full: 3 cột JSON (Definition, Grammar, Example)
            # Thay vì HTML như trước
            self.cursor.execute("""
                CREATE TABLE IF NOT EXISTS entries (
                    id INTEGER PRIMARY KEY,
                    headword TEXT NOT NULL,
    ...
    def insert_batch(self, entries: list, lookups: list):
        if entries:
            ...
            if self.config.is_tiny_mode:
                ...
                # Mini Mode: Insert vào 3 cột JSON
                self.cursor.executemany(
                    "INSERT INTO entries (id, headword, headword_clean, definition_json, grammar_json, example_json) VALUES (?,?,?,?,?,?)",
                    entries
                )
