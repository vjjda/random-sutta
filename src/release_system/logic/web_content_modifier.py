...
def _update_file(file_path: Path, pattern: str, replacement: str) -> bool:
    ...
    try:
        ...
        # Chỉ replace nếu tìm thấy pattern
        if re.search(pattern, content):
            new_content = re.sub(pattern, replacement, content)
            with open(file_path, "w", encoding="utf-8") as f:
                f.write(new_content)
            return True
        return False # Pattern not found is not necessarily error, but warning
    except Exception as e:
        logger.error(f"❌ Error updating {file_path.name}: {e}")
        return False

# ... (Giữ nguyên inject_version_into_sw) ...
...
def patch_sw_assets_for_offline(target_dir: Path) -> bool:
    ...
    # Tìm dòng chứa path file module
...
# ... (Giữ nguyên _patch_html_assets, patch_online_html, patch_offline_html) ...
