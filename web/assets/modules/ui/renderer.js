...
function getDisplayInfo(uid, metaMap) {
    ...
    // Format ID đẹp (ví dụ an1.1 -> AN 1.1)
...
export async function renderSutta(suttaId, data, options = {}) {
  ...
  // Tính toán Navigation (Dùng chung cho cả Branch và Leaf)
  // navigator.js đã hỗ trợ tìm kiếm theo Depth cho Branch
  ...
  // Render UI chung (Header + Content + Bottom Nav)
  // [UPDATED] Giờ đây Branch cũng sẽ có Nav Button
  ...
  // Chỉ setup TOH (Mục lục nội bộ) nếu là Leaf (Bài đọc)
  if (!data.isBranch) {
